...
async function ensureServer() {
  ...
  if (baseUrlEnv) {
    ...
    console.warn(`Configured OPENCODE_BASE_URL is not reachable: ${baseUrlEnv}. Starting a local server...`);
  }
  let server;
  ...
  const hint = "Failed to start opencode server. Stop other opencode instances or set OPENCODE_BASE_URL to an existing server (e.g., http://127.0.0.1:4096). Check logs under %USERPROFILE%/.local/share/opencode/log.";
...
async function createAnalysisClient() {
...
var DEFAULT_CONFIG = {
  ...
  agent: {
    "code-analysis": {
      description: "Read-only expert that maps architecture, APIs, and critical flows for this repository.",
      mode: "primary",
...
async function pathStats(target) {
  try {
...
`);
    ...
    if (normalized.includes("overload") || normalized.includes("rate limit")) {
      const waitMs = Math.min(1e4, 2000 * Math.pow(2, attempt));
...
async function askOtherCodebase(params) {
  ...
  const sessionRes = await client.session.create({
    body: {
      title: "Code analysis for " + path2.basename(setup.projectRoot)
    },
  ...
  if (setup.createdAgents || setup.createdConfig) {
    ...
    summarySegments.push("Initialized " + created.join(" and ") + " in " + setup.projectRoot + ".");
  }
...
async function startCodeAnalysisServer() {
  const server = createCodeAnalysisServer();
  const transport = new StdioServerTransport;
  await server.connect(transport);
  console.error("Code analysis MCP server is running on stdio. Use the ask-other-codebase tool for deep repository questions.");
}
